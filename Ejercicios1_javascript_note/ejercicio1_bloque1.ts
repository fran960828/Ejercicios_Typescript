...
let helados: [string, string, string];
helados = ["fresa", "mango", "coco"];
console.log(
  `Me gusta el helado de ${helados[0]}, ${helados[1]} y ${helados[2]}`
);
...
let meses30: string[] = ["Abril", "Junio", "Septiembre", "Noviembre"];
let meses28: [string] = ["Febrero"];
...
let familia: string[] = ["padre", "madre", "Hermano", "Hermana", "Hijo"];
...
let segundosTotales: number = 3620; // cantidad total de segundos a convertir

// Cálculos
const horas: number = Math.floor(segundosTotales / 3600); // cuántas horas completas hay
const restoDespuesDeHoras: number = segundosTotales % 3600; // resto tras quitar horas

const min: number = Math.floor(restoDespuesDeHoras / 60); // minutos restantes
const seg: number = restoDespuesDeHoras % 60; // segundos restantes

// Mostrar resultado
console.log(
  `${segundosTotales} segundos = ${horas} hora(s) ${min} minuto(s) ${seg} segundo(s)`
);
...
for (let i = 0; i < 3; i++) {
  ...
  dinero -= gasto;
  console.log(
    `${servicio} tiene un coste de ${gasto} euros, tu saldo actual es ${dinero}`
  );
}
