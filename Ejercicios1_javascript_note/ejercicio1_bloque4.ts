...
async function main() {
  const busqueda: string = "mayor";
  const cantidadNumeros: number = 4;
...
function numeroVueltas(pregunta: string): Promise<string> {
  return new Promise((resolve) => {
    rl.question(pregunta, (respuesta:string) => {
      resolve(respuesta);
    });
...
function tiempoVuelta(pregunta: string): Promise<string> {
  return new Promise((resolve) => {
    rl.question(pregunta, (respuesta:string) => {
      resolve(respuesta);
    });
...
async function CalcularPromedio() {
    ...
    promedio=tiempos.reduce((ac:number,valor:number):number => {
        return ac+valor
    },0)
...
async function caraCruz(){
    const conjuntoAleatorio:[string,string]=['cara','cruz']
    let eleccionAleatoria:string|undefined;
    let eleccionJugador= await preguntar('Elegir entre cara|cruz|salir: ')
    let contador:number=0
    let rachaMaxima:number=0;
...
async function numeroFavorito(){
    let favorito:number=Math.floor(Math.random()*11)
    let favoritoUsuario:number=12;
